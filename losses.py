...
#自己改的第一版
class AFLoss(nn.Module):
    def __init__(self, gamma_pos, gamma_neg, hard_negative_ratio=0.2):
        super().__init__()
        self.gamma_pos = gamma_pos
        self.gamma_neg = gamma_neg
    ...
    def forward(self, logits, labels):
        ...
        # 挖掘困难负样本
        negative_loss = loss1 * (1 - labels)
        num_hard_negatives = int(self.hard_negative_ratio * negative_loss.numel())
        hard_negatives, _ = torch.topk(negative_loss.view(-1), num_hard_negatives)

        loss = 1.0 * (loss1.sum(1).mean() + hard_negatives.mean()) + 1.0 * loss2.mean()
        return loss
